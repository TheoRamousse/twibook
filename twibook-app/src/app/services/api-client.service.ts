...
export class ApiClientService extends PersistenceTemplateService {
  ...
  getPostById(id: string): Post {
    let value: Post = null;
    this.http.get<Post>(environment.apiUrl+'posts/'+id).subscribe(post => value=post);
    return value;
  }
  ...
  getUserByIdentifiant(identifiant: string): User {
    let value: User = null;
    this.http.get<User>(environment.apiUrl+'users/'+identifiant).subscribe(user => value=user);
    return value;
  }
